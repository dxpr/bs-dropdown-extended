...
class BootstrapEnhancedDropdowns {
    ...
    log(...args) {
        if (this.options.debug) {
            console.log('[BS-Enhanced-Dropdowns]', ...args);
        }
    }
    
    init() {
        this.log('Initializing enhanced dropdowns');
    ...
    setupDropdowns() {
        this.log('Setting up enhanced dropdowns');
    ...
    initSplitButtonDropdowns() {
        ...
        this.log(`Found ${splitButtons.length} split buttons`);
        
        splitButtons.forEach((splitButton, index) => {
            this.log(`Processing split button ${index + 1}/${splitButtons.length}`);
            ...
            if (!caretButton || !linkElement) {
                this.log('Missing caret button or link element in split button', splitButton);
                return;
            }
            ...
            if (!menu) {
                this.log('Could not find dropdown menu for split button', splitButton);
                return;
            }
            
            this.log('Found menu for split button', menu.id || menu);
            ...
            caretButton.addEventListener('click', (event) => {
                this.log('Caret button clicked, toggling dropdown');
            ...
            caretButton.addEventListener('keydown', (event) => {
                if (event.key === 'Enter' || event.key === ' ') {
                    this.log('Caret key press, toggling dropdown');
    ...
    initSubmenuDropdowns() {
        ...
        this.log(`Found ${submenuDropdowns.length} submenu dropdowns`);
        
        submenuDropdowns.forEach((submenu, index) => {
            this.log(`Processing submenu ${index + 1}/${submenuDropdowns.length}`);
            ...
            if (!toggleElement) {
                this.log('No toggle element found for submenu', submenu);
                return;
            }
            ...
            if (!menu) {
                this.log('No dropdown menu found for submenu', submenu);
                return;
            }
            ...
            // Initialize Bootstrap Dropdown
            this.log('Initializing Bootstrap Dropdown for submenu', toggleElement.id || toggleElement);
